...
CodePath=${BuildDir}/bin/Fabric/Fabric.Code
...
ClusterManifest="ClusterManifest.SingleMachine.Replaced.xml"

echo "Setting environment for FabricHost & FabricDeployer to run"
export LD_LIBRARY_PATH=${CodePath}:$LD_LIBRARY_PATH

if [ ${SetupHarnessRun} == "false" ] ; then
    echo "Running CoreCLR FabricDeployer on ClusterManifest.SingleMachine.xml"
    ${CodePath}/FabricDeployer.sh /operation:Create /fabricBinRoot:${BinPath} /fabricDataRoot:${DataRoot} /cm:${ClusterManifest}
...
if command -v lttng >/dev/null 2>&1; then
    ${BinPath}/Fabric/DCA.Code/lttng_handler.sh $(cat ${BinPath}/Fabric/DCA.Code/ClusterVersion) $traceDiskBufferSizeInMB $keepExistingSFSessions
else
...
if $sfappsuser_exists; then
   ...
   chown sfappsuser:sfuser /home/sfappsuser
fi
...
if [ ! -f "/usr/bin/cgget" ]; then
    echo "Error: cgroup-tools is not installed. Please run apt-get install cgroup-tools to install cgroup support"
    exit -1
fi
