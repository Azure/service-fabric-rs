...
pub(crate) mod test_utilities {
    ...
    pub unsafe fn check_array_parameter<const N: usize>(
        ...
        actual_len: u32,
        actual_values_start: *const PCWSTR,
    ) {
...
pub(crate) struct MockIFabricClientSettings {
    pub set_security_credentials_mock:
        Box<dyn Fn(*const FABRIC_SECURITY_CREDENTIALS) -> windows_core::Result<()>>,
...
impl MockIFabricClientSettings {
    ...
    pub fn new_with_security_credentials_mock(
        mock: Box<dyn Fn(*const FABRIC_SECURITY_CREDENTIALS) -> windows_core::Result<()>>,
    ) -> Self {
        Self {
